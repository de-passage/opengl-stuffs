...
static constexpr inline int NK_GLFW_TEXT_MAX = 256;
#endif
...
NK_API struct nk_context* nk_glfw3_init(struct nk_glfw* glfw,
                                        GLFWwindow* win,
...
NK_API void nk_glfw3_mouse_button_callback(GLFWwindow* win,
                                           int button,
                                           int action,
                                           int mods);
...
NK_API void nk_glfw3_char_callback(GLFWwindow* win, unsigned int codepoint) {
  struct nk_glfw* glfw = static_cast<nk_glfw*>(glfwGetWindowUserPointer(win));
...
NK_INTERN void nk_glfw3_clipboard_paste(nk_handle usr,
  ...
  struct nk_glfw* glfw = static_cast<nk_glfw*>(usr.ptr);
...
NK_INTERN void nk_glfw3_clipboard_copy(nk_handle usr,
                                       const char* text,
                                       int len) {
  char* str = 0;
  if (!len)
...
NK_API void nk_glfw3_font_stash_end(struct nk_glfw* glfw) {
  ...
  nk_font_atlas_end(
      &glfw->atlas, nk_handle_id((int)glfw->ogl.font_tex), &glfw->ogl.null);
